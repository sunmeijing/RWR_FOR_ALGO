...
def __compute_document_signature(W_telta, Ed, candidates, mentions, tfidf_score, prior_score, prob=PROB):
    # TODO
...
def __compute_entity_signature(W_telta, entity, prob=PROB):
    # TODO
    l = W_telta.shape[0]
    ei = np.array([[0.0]]*l)
    ei[entity[0]] = 1
...
def __semantic_similarity(sig_entity, sig_document, kl_l=distance.KL_L, isKL=True):
    if not isKL:
...
def entity_link(mentions, graph, candidates, tfidf_score, prior, prob=PROB, kl_l=distance.KL_L,  isKL=True):
    ...
    for mention in mentions:
        if len(candidates[mention]) > 1:
            ...
            for entity in candidates[mention]:
                sig_entity = __compute_entity_signature(W_telta, entity, prob=prob)
                # use this cause too many noise
                # if one link has many sub node then it can be very high
                phy_1 = prior[(entity, mention)]
                phy_2 = __semantic_similarity(sig_entity, sig_document, kl_l=kl_l, isKL=isKL)
...
def entity_link_only_with_simrank(mentions, graph, candidates, tfidf_score, prior, prob=PROB, kl_l=distance.KL_L, isKL=True):
    ...
    for mention in mentions:
        if len(candidates[mention]) > 1:
            ...
            max_score_entity = None
            for entity in candidates[mention]:
                score = local_sims[mention[0]][entity[0]]
                print entity[1], ":", mention, score
                if score > max_score:
                    max_score = score
                    max_score_entity = entity
            print "-----------------------------------"
...
def entity_link_only_with_rwr(mentions, graph, candidates, tfidf_score, prior, prob=PROB, kl_l=distance.KL_L, isKL=True):
    ...
    for node in graph.nodes():
        if len(graph.neighbors(node)) == 0:
            # we add self link
            graph.add_edge(node, node, weight=1)
    w_norm = numpy_helper.normalize_matrix(graph)
    SIMLARITY_THREHOLD = 0
    ...
    sig_document = __compute_document_signature(w_norm, Ed, candidates, mentions, tfidf_score, prior, prob=prob)
    ...
    for mention in mentions:
        if len(candidates[mention]) > 1:
            ...
            max_score_entity = None
            for entity in candidates[mention]:
                sig_entity = __compute_entity_signature(w_norm, entity, prob=prob)
                # use this cause too many noise
                # if one link has many sub node then it can be very high
                phy_1 = 0
                phy_2 = __semantic_similarity(sig_entity, sig_document, kl_l=kl_l, isKL=isKL)
...
def entity_link_only_with_prior(mentions, graph, candidates, tfidf_score, prior, prob=PROB, kl_l=distance.KL_L, isKL=True):

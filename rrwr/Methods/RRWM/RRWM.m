...
%% Default Parameters
param = struct( ...
    'c', 0.38, ...                   % prob. for walk or reweighted jump?
...
[rows,cols]=size(prev_score);
cc=zeros(rows,rows);
E=diag(ones(1,rows));
prev_important=prev_score-prev_assign;
twentyfive=prctile(prev_important,25);
fifty=prctile(prev_important,50);
seventyfive=prctile(prev_important,75);
paraArray=[0.15,0.183,0.227,0.25];
paraArray=[0.37,0.4033,04367.,0.47];

distort=(asin(-1)/pi+0.5);
...
% fifty=prctile(prev_important,50);
% seventyfive=prctile(prev_important,75);
head=prctile(prev_important,0);
tail=prctile(prev_important,100);
% head=abs(head);
% tail=abs(tail);
% if(head>tail)
%     tmp=head;
...
for cindex=1:rows
    if(tail==head)
        cc(cindex,cindex)=0.42;
    else
        %percentMine=abs(abs(prev_important(cindex,:))-head)/abs(tail-head);
         percentMine=abs((prev_important(cindex,:))-min(head,tail))/abs(tail-head);
        %cc(cindex,cindex)=0.25+percentMine*0.1;
        cc(cindex,cindex)=0.25+  (asin(percentMine*2-1)/pi+0.5)*0.1;
        %cc(cindex,cindex)=0.15+  ((sin(percentMine*pi-pi/2)+1)/2)*0.1;
    end
end

%     for cindex=1:rows
%          if(prev_score(cindex,:)-prev_assign(cindex,:)<twentyfive)
%             cc(cindex,cindex)=paraArray(:,1);
%          else if(prev_score(cindex,:)-prev_assign(cindex,:)<fifty)
%             cc(cindex,cindex)=paraArray(:,2);
%              else if(prev_score(cindex,:)-prev_assign(cindex,:)<seventyfive)
%                   cc(cindex,cindex)=paraArray(:,3);
%                  else
%                      cc(cindex,cindex)=paraArray(:,4);
%                  end
%              end
%          end
% end
    ...
    %cur_score = Mo * ( c*prev_score + (1-c)*prev_assign );
    cur_score = Mo * ( cc*prev_score + (E-cc)*prev_assign );
